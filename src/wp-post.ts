...
class WPPost extends HTMLElement {
  ...
  renderListView() {
    ...
    if (thumbnailUrl) {
      this.querySelector('.post-thumbnail')!.setAttribute(
        'src',
  ...
  renderSingleView() {
    ...
    if (thumbnailUrl) {
      this.querySelector('.post-thumbnail')!.setAttribute(
        'src',
        wpcomGetThumbnailUrl(thumbnailUrl, featuredImageParams)
      );
    }
    this.querySelector(
      '.post-published'
  ...
  render() {
    return 'single' === this.getAttribute('view')
